...
public class CollisionScript : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            RCC_CarControllerV3 playerCarController = GameplayManager.instance
                .playerCar[DataController.instance.GetSelectedVehicle()].GetComponent<RCC_CarControllerV3>();
    ...
    private void HandleStreetLightCollision(GameObject streetLight)
    {
        ...
        if (streetLightRigidbody != null)
        {
            ...
            {
                if (gameplayManagerFreeMode)
                {
                    streetLightRigidbody.AddRelativeForce(-gameplayManagerFreeMode.impactDirection.up * 10f,
                        ForceMode.Impulse);
                }
                else
                {
                    streetLightRigidbody.AddRelativeForce(-gameplayManager.impactDirection.up * 10f,
                        ForceMode.Impulse);
                }
        ...
        if (waypointMover && !waypointMover.isDead && other.gameObject.activeInHierarchy)
        {
            ...
            if (waypointMover && !waypointMover.isDead && other.gameObject.activeInHierarchy &&
                Vector3.Distance(other.transform.position,
                    GameplayManager.instance.playerCar[DataController.instance.GetSelectedVehicle()].transform
                        .position) > 5)
            {
