...
public class ShootBehaviour : GenericBehaviour
{
	...
	private void Update()
	{
		...
		if (ControlFreak2.CF2Input.GetAxisRaw(shootButton) != 0 && !isShooting && activeWeapon > 0 && burstShotCount == 0)
		{
			isShooting = true;
			ShootWeapon(activeWeapon);
		}
		else if (isShooting && ControlFreak2.CF2Input.GetAxisRaw(shootButton) == 0)
		{
			isShooting = false;
		}
		// Handle reload weapon action.
		else if (ControlFreak2.CF2Input.GetButtonUp(reloadButton) && activeWeapon > 0)
		{
		...
		else if (ControlFreak2.CF2Input.GetButtonDown(dropButton) && activeWeapon > 0)
		{
		...
		{
			if ((ControlFreak2.CF2Input.GetAxisRaw(changeButton) != 0 && !isChangingWeapon))
			{
				isChangingWeapon = true;
				int nextWeapon = activeWeapon + 1;
				ChangeWeapon(activeWeapon, (nextWeapon) % weapons.Count);
			}
			else if (ControlFreak2.CF2Input.GetAxisRaw(changeButton) == 0)
			{
			...
			if (shotDecay <= 0.4f)
			{
				...
				if (activeWeapon > 0)
				{
					...
					if (shotDecay <= (0.4f - 2 * Time.deltaTime))
					{
						...
						if (weapons[activeWeapon].mode == InteractiveWeapon.WeaponMode.AUTO && ControlFreak2.CF2Input.GetAxisRaw(shootButton) != 0)
						{
